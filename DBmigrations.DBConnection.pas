...
type
  TDatabaseType = (dtMySQL, dtPostgreSQL, dtSQLServer, dtSQLite, dtOracle, dtFirebird, dtOther);
...
begin
  ...
  try
    if FDriverID <> '' then Params.Add('DriverID=' + FDriverID);
    if FServer <> '' then Params.Add('Server=' + FServer);
    if FPort > 0 then Params.Add('Port=' + FPort.ToString);
    if FDatabase <> '' then Params.Add('Database=' + FDatabase);
    if FUserName <> '' then Params.Add('User_Name=' + FUserName);
    if FPassword <> '' then Params.Add('Password=' + FPassword);
    if FCharacterSet <> '' then Params.Add('CharacterSet=' + FCharacterSet);
    if FProtocol <> '' then Params.Add('Protocol=' + FProtocol);
